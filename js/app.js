...
function header() {
  ...
  // Обработчик события изменения размера окна или скролла
  ...
  //   // Применяем стили в зависимости от состояния адресной строки
...
function intro() {
  ...
  function swiperNavigation(item) {
    ...
    let currentIndex = 0;
    let autoplay;
    let isClicked = false; // Флаг для предотвращения двойного клика
    const clickDelay = 1600; // Задержка перед повторным кликом
    items.forEach((e, i) => {
      e.addEventListener("click", () => {
        if (isClicked) return; // Игнорируем клик, если он уже был
        isClicked = true; // Запрещаем повторные клики

        currentIndex = i; // Обновляем currentIndex на индекс элемента, на который кликнули
        resetAutoplay(); // Перезапускаем автоплей с учетом клика
        updateClasses(); // Обновляем классы после изменения currentIndex

        setTimeout(() => {
          isClicked = false; // Разрешаем клики после задержки
        }, clickDelay);
      });
    });
    ...
    function updateClasses() {
      // Удаляем текущие классы с элементов
    ...
    function nextSlide() {
      currentIndex = (currentIndex + 1) % item.length; // Инкрементируем индекс с циклированием
      updateClasses(); // Обновляем классы
      resetAutoplay();
    }
